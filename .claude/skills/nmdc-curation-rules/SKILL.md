...
7. **Validate before commit.** For ontology-bearing slots (env triad, taxon, etc.), the slot-specific skill (`nmdc-env-triad`, `nmdc-taxon-resolution`) names the validators that must pass before a commit, and the batch validator `nmdc-ingest-validate-terms` (linkml-term-validator: CURIE exists / not obsolete, canonical label, anchor class, NMDC value set) writes its verdicts into each report row's `validator` dict — run it on the final deliverable. An **error-level** validator failure (missing or obsolete CURIE, label mismatch: `info_ok` / `label_ok` false) means revert to sentinel and write `outcome: "validator_rejected"` to the report — never silently downgrade. A **warning-level** verdict (`anchor_ok` / `valueset_ok` false) is a prompt to reconsider; keeping the term is allowed only with the justification recorded in the row's `evidence`.
