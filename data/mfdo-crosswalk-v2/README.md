...
- **`mfd_nominatim_geocode.tsv`**: per-coordinate Nominatim reverse-geocoding. Under policy discussion; not used by `apply_crosswalk.py`.
...
- **`els_allowlist.tsv`**: 1,510 ENVO CURIEs valid as `env_local_scale` values, generated by `generate_els_allowlist.py`. Note: `build_ontology_crosswalk.py` documents 1,533 classes; the difference is ENVO version.
...
`build_ontology_crosswalk.py` documents 1,533 classes (from the ENVO version used during initial curation). `els_allowlist.tsv` has 1,510. The difference is ENVO version; both are correct for their respective snapshots. Regenerate `els_allowlist.tsv` after an ENVO release to stay current.
...
biomes and materials. The standalone `els_allowlist.tsv` artifact covers this for
verification; the script's embedded copy and the TSV may diverge when ENVO releases
a new version.
