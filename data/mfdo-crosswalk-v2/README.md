...
- **`mfdo_nmdc_crosswalk.tsv`**: primary output (284 rows). A `row_type` column marks each row as `ontology_leaf` (279 genuine MFD leaves) or `biosample_reconciliation` (5 rows added so the join is total over all 10,875 biosamples).
...
- **`apply_crosswalk.py`**: applies the crosswalk and CORINE/WorldCover-based ELS refinement to a biosample TSV. Run `--help` for options. Implements `JOIN_RECIPE.md`.
