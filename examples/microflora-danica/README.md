...
The `Extraction` and `LibraryPreparation` steps of the chain (see **Material-processing chain** above) — both in this collection. `01`/`02` are the consecutive steps of the `02_forest` (MFD00001) WGS chain; `03`/`04` are amplicon `LibraryPreparation`s showing the **domain-aware `target_gene`** parsed from the SRA `DESIGN_DESCRIPTION` — a bacterial target → `16S_rRNA`, a eukaryotic target → `18S_rRNA`.
...
| `03_amplicon_library_preparation.json` | `nmdc:libprp-99-2c37e48b` | extracted-DNA ProcessedSample → library ProcessedSample (consumed by `data_generation_set/02`) | **Bacterial amplicon** `LibraryPreparation`: `library_strategy=AMPLICON`, `library_selection=PCR`, `lib_layout=single`, and **`target_gene="16S_rRNA"`** (design "...amplify bacterial rRNA operons" → bacterial SSU = 16S). No `protocol_link` (the amplicon design cites no DOI) — the inverse of `02`. |
| `04_amplicon_eukaryotic_library_preparation.json` | `nmdc:libprp-99-654373f2` | extracted-DNA ProcessedSample → library ProcessedSample | **Eukaryotic amplicon** `LibraryPreparation` (MFD's `pb_eukoperon` libraries): **`target_gene="18S_rRNA"`** (design "...amplify eukaryotic rRNA operons" → eukaryotic SSU = 18S, *not* 16S). 450 MFD libraries are 18S; this is the case the domain-aware parser fixes. |
